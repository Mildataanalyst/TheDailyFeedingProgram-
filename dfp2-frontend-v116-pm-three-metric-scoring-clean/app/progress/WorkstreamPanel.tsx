...
type AdminEvidenceDraft = Record<MetricKey, { text: string; linksText: string }>;
...
function emptyAdminEvidence(): AdminEvidenceDraft { return { child_progression: { text: '', linksText: '' }, learning_model: { text: '', linksText: '' }, development_ecosystem: { text: '', linksText: '' } }; }
...
function submittedRows(pm?: PmData) { return Object.entries(pm?.responses || {}).filter(([, row]) => (row as ResponseRow)?.submitted) as Array<[string, ResponseRow]>; }
...
function latestSubmitMs(pm?: PmData) { const times = submittedRows(pm).map(([, row]) => new Date(String(row.submitted_at)).getTime()).filter(Number.isFinite); return times.length ? Math.max(...times) : 0; }
function avgPace(pm?: PmData) { const times = submittedRows(pm).map(([, row]) => new Date(String(row.submitted_at)).getTime()).filter(Number.isFinite).sort((a, b) => a - b); if (times.length < 2) return '—'; const diffs = times.slice(1).map((time, index) => Math.max(1, Math.round((time - times[index]) / 1000))); const avg = Math.round(diffs.reduce((a, b) => a + b, 0) / diffs.length); return formatDuration(avg); }
function latestSubmittedIndex(pm?: PmData) { const tasks = pm?.tasks || []; const raw = Number((pm as any)?.last_submitted_task_index); if (Number.isFinite(raw) && raw >= 0 && raw < tasks.length) return raw; const rows = submittedRows(pm); if (!rows.length) return 0; let best = 0; let bestMs = -1; rows.forEach(([idx, row]) => { const ms = new Date(String(row.submitted_at || row.global_saved_at || '')).getTime(); const n = Number(idx); if (Number.isFinite(n) && (Number.isFinite(ms) ? ms : 0) >= bestMs) { best = n; bestMs = Number.isFinite(ms) ? ms : 0; } }); return Math.min(Math.max(best, 0), Math.max(0, tasks.length - 1)); }
...
function reviewerMode(pm?: PmData) { const rows = submittedRows(pm).map(([, row]) => row); if (!rows.length) return 'Not started'; const avgWords = Math.round(rows.reduce((sum, row) => sum + wordCount(row.reason || row.ngo_description || ''), 0) / Math.max(1, rows.length)); if (avgWords >= 35) return 'Essay Department'; if (avgWords <= 3) return 'Minimalist menace'; return rows.length >= 5 ? 'Speed Reviewer' : 'Warming up'; }
...
export default function WorkstreamPanel({ stateName }: { stateName: string }) {
  ...
  function validationMessage() {
    if (isDetails) return '';
    if (!rank) return 'Select an overall rank.';
    if (rank >= 4 && wordCount(reason) < 1) return 'Add one quick reason for the existing overall rank.';
    if (rank === 3 && wordCount(reason) < 1) return 'Add one quick note for the existing overall rank.';
    for (const metric of METRIC_DEFINITIONS) {
      const row = metricScores[metric.key];
      if (!row?.rank) return `Select a rank for ${metric.title}.`;
      if (String(row.reason || '').trim().length < 100) return `${metric.title}: reason must be at least 100 characters.`;
    }
    return '';
  }
  ...
  async function submitDecision() {
    ...
    if (error) { setMsg(error); if (!isDetails && error.includes(': reason')) startRerank(); return; }
    const prevMs = latestSubmitMs(pm);
    const rankLabel = isDetails ? 'Details' : RANKS[rank].short;
    const nowIso = new Date().toISOString();
    const localResponse: ResponseRow = {
      decision: isDetails ? 'Details' : String(rank),
      rank: isDetails ? undefined : rank,
      rank_label: rankLabel,
      reason,
      metric_scores: isDetails ? undefined : metricScores,
      ngo_description: description,
      contact_number: isDetails ? contactNumber : '',
      referral_source: isDetails ? referralSource : '',
      referral_poc: isDetails ? contactNumber : '',
      submitted: true,
      submitted_at: nowIso,
      global_saved: true,
      global_saved_at: nowIso,
    };
    const newDone = response.submitted ? done : done + 1;
    const delta = prevMs ? Math.max(1, Math.round((Date.now() - prevMs) / 1000)) : 0;
    setLastBadge(paceBadge(delta));
    setLastPaceSeconds(delta || null);
    setLastQuality(qualityLabel(isDetails ? description : reason, isDetails));
    setStreak(value => response.submitted ? value : value + 1);
    patchLocalResponse(localResponse);
    setReranking(false);
    setMsg(response.submitted ? 'Re-ranking saved. Existing overall rank and all three metric ranks are updated.' : reactionText(prevMs, newDone));
    burstConfetti(newDone === 1 ? 130 : newDone === 5 ? 180 : newDone === total ? 170 : 55);
    maybeShowMilestone(newDone);
    const autoReview = newDone <= 5 && !response.submitted;
    const payload = {
      pm: selectedPM,
      ...
      ngo_description: description,
      contact_number: localResponse.contact_number,
      referral_source: localResponse.referral_source,
      referral_poc: localResponse.referral_poc,
    };
    const base = backendBase();
    if (base) {
      try {
        const result = await backendFetch(`${base}/workstream/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  ...
  function updateAdminEvidence(metricKey: MetricKey, patch: Partial<{ text: string; linksText: string }>) { setAdminEvidence(old => ({ ...old, [metricKey]: { ...old[metricKey], ...patch } })); }
                ...
                <>
                  ...
                  {task ? (
                    <div className="workstream-task-card pm-ranking-task-card">
                      ...
                      {!isDetails ? (
                        <>
                          <section className="legacy-ranking-section">
                            <div className="legacy-ranking-label"><span>Existing overall ranking</span><small>Kept exactly as before</small></div>
                            <div className="rank-slider-wrap">
                              <div className="rank-top"><b>{rank}</b><span>{RANKS[rank].short}</span></div>
                              <input className={`rank-slider rank-${rank}`} type="range" min="1" max="5" step="1" value={rank} disabled={editLocked} onChange={event => setRank(Number(event.target.value))} />
                              <div className="rank-scale"><span>1</span><span>2</span><span>3</span><span>4</span><span>5</span></div>
                              <p className="rank-copy">{RANKS[rank].line}</p>
                            </div>
                            <textarea className="workstream-textarea" value={reason} disabled={editLocked} onChange={event => setReason(event.target.value)} placeholder="Why this overall rank? Write what comes to mind. Hinglish, fragments, messy notes — all fine." />
                            <div className="word-helper">{wordCount(reason)} words · existing reason format retained.</div>
                          </section>

                          <section className="metric-ranking-section" ref={metricSectionRef}>
                            <div className="metric-ranking-intro">
                              <div><span className="workstream-kicker">Three-dimension scoring</span><h3>Rank each metric separately</h3><p>Use the same 1–5 scale for all three. Every metric reason needs at least 100 characters.</p></div>
                              {response.submitted && <button type="button" className="metric-rerank-btn" onClick={startRerank}>↻ Re-rank</button>}
                            </div>
                            {reranking && <div className="rerank-notice">Re-ranking mode is open. Change any metric score or reason, then choose “Save re-ranking”.</div>}
                      ...
                      <div className="workstream-actions">
                        <button className="primary-red glow-action" onClick={submitDecision} disabled={editLocked}>{response.submitted && !isDetails ? 'Save re-ranking' : 'Submit'}</button>
                        {response.submitted && !isDetails && <button className="ghost-btn" type="button" onClick={startRerank}>Re-rank</button>}
                        <button className="ghost-btn" onClick={() => runReview('selected')} disabled={!response.submitted}>Response review</button>
                        <button className="ghost-btn" onClick={deleteResponse} disabled={!response.submitted || editLocked}>Delete response</button>
                      </div>
                      <div className="live-reaction"><b>Saved</b><span>{msg || (response.submitted ? `Saved at ${niceTime(response.submitted_at)}. Edit and submit again anytime.` : 'Submit to save globally.')}</span></div>
                    </div>
                ...
                <div className="admin-subsection metric-evidence-admin">
                  ...
                  {METRIC_DEFINITIONS.map(metric => (
                    <section className="admin-metric-evidence-block" key={metric.key}>
                      ...
                      <label className="admin-field"><span>Evidence</span><textarea value={adminEvidence[metric.key].text} onChange={event => updateAdminEvidence(metric.key, { text: event.target.value })} placeholder={`Evidence for ${metric.title.toLowerCase()}`} /></label>
                      <label className="admin-field"><span>Links — one per line</span><textarea value={adminEvidence[metric.key].linksText} onChange={event => updateAdminEvidence(metric.key, { linksText: event.target.value })} placeholder={'Annual report | https://example.org/report\nProgramme page | https://example.org/programme'} /></label>
                    </section>
                  ))}
                </div>
              )}
